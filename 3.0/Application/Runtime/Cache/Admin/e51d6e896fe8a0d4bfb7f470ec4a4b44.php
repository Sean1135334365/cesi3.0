<?php if (!defined('THINK_PATH')) exit();?><li id="table2">
    ...
    <?php if($ht_page_number['page'] != 1 ): ?><a href="javascript:;" class="ajax_page" page="<?php echo ($ht_page_number['page']-1); ?>" dj="3">«</a><?php endif; ?>
    <?php if(($ht_page_number['count'] > 10) AND ($ht_page_number['page'] > 3) ): ?><a href="javascript:;">...</a><?php endif; ?>
    <?php $__FOR_START_1728543097__=1;$__FOR_END_1728543097__=$ht_page_number['count'];for($i=$__FOR_START_1728543097__;$i <= $__FOR_END_1728543097__;$i+=1){ if(($i > $ht_page_number['page'] - 3) AND (($i < $ht_page_number['page'] + 3) OR (($ht_page_number['page'] <= 3) AND ($i <= 10)))): if($i == $ht_page_number['page'] ): ?><a href="javascript:;" class="current"><?php echo ($i); ?></a>
                <?php else: ?>
    ...
    <?php if($ht_page_number['page'] < $ht_page_number['count'] ): ?><a href="javascript:;" class="ajax_page" page="<?php echo ($ht_page_number['page']+1); ?>" dj="3">»</a><?php endif; ?>
</div>
...
<script>
    $('#hidden').on('change',function(){
           ...
           console.log(arr);
           /*修改地址*/
           $.post("http://cesi3.0.zhiyujinrong.com/index.php/Admin/Product/parameter_list_page.html",{type:arr[0],page:arr[1]},function(res){
               if(arr[0] === '1'){
                   $("#architecture").html('');
    ...
    function edit_list3(type){
        layer.open({
            ...
            closeBtn: 0,
            shadeClose: true,
            content: ["<?php echo U('Admin/Product/wechat_update',array('type'=>'3','page'=>$ht_page_number['page']));?>",'no']
